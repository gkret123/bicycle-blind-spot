...
def send_cmd_wait_done(ser: serial.Serial, cmd: str, timeout_s: float) -> bool:
    ...
    ser.write((cmd + "\r\n").encode())  # CRLF is robust on Linux
    ser.flush()
...
def parse_tlv1_points(payload: bytes):
    """
    Your TLV1 looks like float32 [x,y,z,vel]*N with extra footer bytes (often 8).
    """
    L = len(payload)
...
def run_metrics(data_port: str, json_lines: bool):
    print(f"== Streaming on {data_port} @ 921600 ==")

    buf = bytearray()

    # Sticky cluster state
    prev_center = None
    y_hist = deque(maxlen=14)
    v_hist = deque(maxlen=14)

    # Detection thresholds (human test mode)
    v_thresh = 0.35      # m/s (radial) toward radar
    min_drop_m = 0.25    # m drop over history window
    need_len = 8
    ...
    with serial.Serial(data_port, 921600, timeout=0.05) as ser:
        while True:
            ...
            tlv_types_seen = []  # helps when you later enable tracker TLVs
            ...
            for _ in range(num_tlvs):
                ...
                tlv_types_seen.append(int(tlv_type))
                ...
                if tlv_type == 1:
                    ...
                    if pts is not None and pts.size > 0:
                        ...
                        if clus:
                            ...
                            # The three outputs you want:
                            rng = float(np.hypot(x_med, y_med))                 # meters
                            az_deg = float(np.degrees(np.arctan2(x_med, y_med))) # deg (0=fwd, +right, -left)
                            ...
                            if len(y_hist) >= need_len:
                                y_drop = float(y_hist[0] - y_hist[-1])
                                v_ok = (np.median(v_hist) < -v_thresh)
                                approaching = bool(v_ok and (y_drop > min_drop_m))

                            out = {
                                "range_m": rng,
                                "azimuth_deg": az_deg,
                                "radial_velocity_mps": v_med,
                                "y_m": y_med,
                                "x_m": x_med,
                                "y_drop_m": y_drop,
                                "approaching": approaching,
                                "cluster_points": int(clu.shape[0]),
                                "tlvs": tlv_types_seen,
                            }

                offset += (tlv_len - 8)

            # decay history if nothing tracked
            if not got_cluster:
                prev_center = None
                if len(y_hist) > 0:
                    y_hist.popleft()
            ...
            if now - last_print > 0.5:
                if out is None:
                    msg = {"status": "NO_TARGET", "tlvs": tlv_types_seen}
                    print(json.dumps(msg) if json_lines else f"NO_TARGET | tlvs={tlv_types_seen}")
                else:
                    if json_lines:
                        print(json.dumps(out))
                    else:
                        print(
                            f"{'APPROACHING' if out['approaching'] else 'clear':>10} | "
                            f"range={out['range_m']:.2f} m | "
                            f"az={out['azimuth_deg']:+.1f} deg | "
                            f"v_r={out['radial_velocity_mps']:+.2f} m/s | "
                            f"pts={out['cluster_points']} | tlvs={out['tlvs']}"
                        )
                last_print = now
...
def main():
    ...
    ap.add_argument("--json", action="store_true", help="Print JSON lines (easy to log/pipe)")
    a = ap.parse_args()

    configure_sensor(a.cfg)
    run_metrics(a.data, json_lines=a.json)
